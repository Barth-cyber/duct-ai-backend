...
const ALLOWED_ORIGINS = process.env.ALLOWED_ORIGINS ? process.env.ALLOWED_ORIGINS.split(',').map((origin) => origin.trim()).filter(Boolean) : [
...
const SYSTEM_PROMPT = `You are Duct AI, the luxury interior design advisor for Interior Duct Ltd. Personality: sophisticated, warm. Expertise: chairs, tables, doors, custom furniture, materials (oak, marble, velvet, brass). Always end with a follow-up question. Never reveal the underlying AI model.`;
...
function sanitizeMessages(messages) {
    ...
    const cleaned = messages
        .map((message) => {
            ...
            return { role, content: sanitizeText(message.content || message.text || '') };
        })
        .filter(Boolean);
    const last24 = cleaned.slice(-24);
    if (last24.length > 0 && last24[0].role === 'user') {
...
app.get('/health', (req, res) => {
    res.json({ status: 'ok', service: 'duct-ai-backend', model: MODEL, keySet: Boolean(GEMINI_API_KEY) });
});
...
app.post('/api/chat', async (req, res) => {
    ...
    const payload = { prompt: { messages: prompt }, temperature: 0.75, maxOutputTokens: 512 };
    try {
        ...
        const reply = Array.isArray(data?.candidates) && data.candidates[0]?.content ? data.candidates[0].content.map((chunk) => chunk.text || '').join('') : typeof data?.output?.text === 'string' ? data.output.text : '';
